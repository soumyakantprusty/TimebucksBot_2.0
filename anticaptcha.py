...
class AntiCaptcha:
    ...
    def callanticaptcha(self):
        ...
        #print(os.environ["anticaptcha_api_key"])
                ...
                self.bot_driver.execute_script("""document.getElementById("g-recaptcha-response").innerHTML = arguments[0]""", g_response)
                time.sleep(10)
                self.bot_driver.execute_script('var element=document.getElementById("g-recaptcha-response"); element.style.display="none";')
        ...
        #print(os.environ["anticaptcha_api_key"])
                ...
                self.bot_driver.execute_script(
                    'var element=document.getElementById("g-recaptcha-response-1"); element.style.display="block";')
                time.sleep(10)
                sitekey = self.bot_driver.find_element(By.CLASS_NAME, 'g-recaptcha').get_attribute('data-sitekey')
                # sitekey_clean = sitekey.split('" data-callback')[0].split('data-sitekey="')[1]
                print(sitekey)
                ...
                print("Trying to paste the response")
                self.bot_driver.execute_script(
                    """document.getElementById("g-recaptcha-response-1").innerHTML = arguments[0]""", g_response)
                time.sleep(10)
                self.bot_driver.execute_script(
                    'var element=document.getElementById("g-recaptcha-response-1"); element.style.display="none";')
                print("submitting the response ")
                time.sleep(10)
